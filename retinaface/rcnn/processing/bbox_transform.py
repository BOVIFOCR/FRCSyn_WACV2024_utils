...
def bbox_overlaps_py(boxes, query_boxes):
    ...
    # overlaps = np.zeros((n_, k_), dtype=np.float)  # original
    overlaps = np.zeros((n_, k_), dtype=float)       # Bernardo
    for k in range(k_):
...
def nonlinear_pred(boxes, box_deltas):
    ...
    # boxes = boxes.astype(np.float, copy=False)  # original
    boxes = boxes.astype(float, copy=False)       # Bernardo
